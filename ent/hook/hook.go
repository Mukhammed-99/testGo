...
// The TtFunc type is an adapter to allow the use of ordinary
// function as Tt mutator.
type TtFunc func(context.Context, *ent.TtMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TtFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TtMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TtMutation", m)
	}
	return f(ctx, mv)
}
