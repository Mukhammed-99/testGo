...
// QueryTtID chains the current query on the "tt_id" edge.
func (pq *PetQuery) QueryTtID() *TtQuery {
	query := &TtQuery{config: pq.config}
...
// WithTtID tells the query-builder to eager-load the nodes that are connected to
// the "tt_id" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PetQuery) WithTtID(opts ...func(*TtQuery)) *PetQuery {
	query := &TtQuery{config: pq.config}
...
func (pq *PetQuery) sqlAll(ctx context.Context) ([]*Pet, error) {
	...
	if pq.withTtID != nil || pq.withOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withTtID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Pet)
		for i := range nodes {
			if fk := nodes[i].id_tt; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(tt.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "id_tt" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.TtID = n
			}
		}
	}
